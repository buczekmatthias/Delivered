 ...
 */
final class Version20201217131200 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE INDEX IDX_5F7E9CF4A5905F5A ON messages_user (messages_id)');
        $this->addSql('CREATE INDEX IDX_5F7E9CF4A76ED395 ON messages_user (user_id)');
        $this->addSql('DROP INDEX IDX_232710AEF624B39D');
        $this->addSql('DROP INDEX IDX_232710AED23057BC');
        $this->addSql('CREATE TEMPORARY TABLE __temp__invitations AS SELECT id, sender_id, to_who_id, requested_at, status, seen FROM invitations');
        ...
        $this->addSql('INSERT INTO invitations (id, sender_id, to_who_id, requested_at, status, seen) SELECT id, sender_id, to_who_id, requested_at, status, seen FROM __temp__invitations');
        ...
        , city VARCHAR(300) DEFAULT NULL COLLATE BINARY, birthday_date DATE DEFAULT NULL, joined_at DATETIME NOT NULL, last_activity DATETIME DEFAULT NULL, image VARCHAR(1000) DEFAULT NULL, CONSTRAINT FK_8D93D64949CA8337 FOREIGN KEY (friends_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__invitations AS SELECT id, sender_id, to_who_id, requested_at, status, seen FROM invitations');
        $this->addSql('DROP TABLE invitations');
        $this->addSql('CREATE TABLE invitations (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, sender_id INTEGER NOT NULL, to_who_id INTEGER NOT NULL, requested_at DATETIME NOT NULL, status BOOLEAN NOT NULL, seen BOOLEAN NOT NULL)');
        $this->addSql('INSERT INTO invitations (id, sender_id, to_who_id, requested_at, status, seen) SELECT id, sender_id, to_who_id, requested_at, status, seen FROM __temp__invitations');
        $this->addSql('DROP TABLE __temp__invitations');
        $this->addSql('CREATE INDEX IDX_232710AEF624B39D ON invitations (sender_id)');
        $this->addSql('CREATE INDEX IDX_232710AED23057BC ON invitations (to_who_id)');
        ...
        , city VARCHAR(300) DEFAULT NULL, birthday_date DATE DEFAULT NULL, joined_at DATETIME NOT NULL, last_activity DATETIME DEFAULT NULL, image VARCHAR(500) DEFAULT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, friends_id, login, roles, password, email, name, city, birthday_date, joined_at, image, last_activity) SELECT id, friends_id, login, roles, password, email, name, city, birthday_date, joined_at, image, last_activity FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649AA08CB10 ON user (login)');
        $this->addSql('CREATE INDEX IDX_8D93D64949CA8337 ON user (friends_id)');
